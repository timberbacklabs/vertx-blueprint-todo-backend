...
public abstract class RestfulApiVerticle extends AbstractVerticle {
    ...
    protected Completable createHttpServer(Router router, String host, int port) {
        return vertx.createHttpServer()
                ...
                .rxListen(port, host)
//                .doOnSuccess(httpServer -> LOGGER.info("HOST:PORT " + host + ":" + port + " " + httpServer.toString()))
                .ignoreElement();
    }
            ...
            asyncResult.subscribe(
                    response -> {
                        if (response.isPresent()) {
                            LOGGER.info("RESPONSE_GET " + response.get());
//                            converter.apply(response.get());
                            LOGGER.info("BEFORE OK");
                            ok(context);
//                            ok(context, converter.apply(response.get()));
                            LOGGER.info("AFTER OK");
                        } else {
                            notFound(context);
                        }},
                    throwable -> internalError(context, throwable));
